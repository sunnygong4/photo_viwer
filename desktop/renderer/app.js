...
function photoUrl(filename) {
  ...
  // In desktop mode, resolve to full server URL for direct loading
  if (window.__SCLOUD_PHOTO_URL) {
    // Return placeholder; actual URL set async in loadLightboxImage
    return "";
  }
  return API_BASE + p;
}
...
async function loadLightboxImage(index) {
  ...
  if (window.__SCLOUD_PHOTO_URL) {
    const url = await window.__SCLOUD_PHOTO_URL(photoPath(filename));
    lightboxImg.src = url;
  } else {
